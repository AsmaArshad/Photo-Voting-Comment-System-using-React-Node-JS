...
function Dropzone(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
    const res = await fetch('http://localhost:4700/image', {
      method: 'POST',
      body: formData,
    })
  ...
  return (
    <section className="container">
      ...
      <div {...getRootProps({ className: 'dropzone' })}>
        <input  {...getInputProps()}  />
        <p style={{ height: '200px', borderStyle: 'dashed', borderColor: 'gray', top: '50%', paddingTop: '21%', paddingLeft: '30px' }}>Drag 'n' drop some files here, or click to select files</p>
      </div>
